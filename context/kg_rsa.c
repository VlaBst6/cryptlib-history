...
static int checkRSAPrivateKeyComponents( INOUT PKC_INFO *pkcInfo )
	...
	const BN_ULONG eWord = BN_get_word( e );
	const BOOLEAN pqReversed = 	( BN_cmp( p, q ) < 0 ) ? TRUE : FALSE;
	...
	/* Verify that u < p, where u was calculated as q^-1 mod p.  As with the 
	   check above, if the p and q values are reversed then we have to change 
	   the check as well */
	if( pqReversed )
		{
		if( BN_cmp( &pkcInfo->rsaParam_u, q ) >= 0 )
			return( CRYPT_ARGERROR_STR1 );
		}
	else
		{

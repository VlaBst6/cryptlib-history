...
*					Copyright Peter Gutmann 2006-2012						*
*																			*
****************************************************************************/
...
#define PKC_CONTEXT		/* Indicate that we're working with PKC contexts */
  ...
  #include "context.h"
  #include "keygen.h"
  #include "asn1.h"			/* For ECC OIDs */
#else
  #include "crypt.h"
  #include "context/context.h"
  #include "context/keygen.h"
  #include "enc_dec/asn1.h"	/* For ECC OIDs */
...
int getECCOidTbl( OUT TYPECAST( OID_INFO ) const void **oidTblPtr,
...
const void *getECCOid( IN_ENUM( CRYPT_ECCCURVE ) \
	...
	int oidTblSize, i, status;

	/* Get the OID for a particular curve */
	status = getECCOidTbl( &oidTbl, &oidTblSize );
	if( cryptStatusError( status ) )
		retIntError_Null();
	for( i = 0; i < oidTblSize && oidTbl[ i ].oid != NULL; i++ )
		{
		if( oidTbl[ i ].selectionID == curveType )
			return( oidTbl[ i ].oid );
		}

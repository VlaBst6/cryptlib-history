...
/* Cryptlib options: Enable use of asm encryption with C key schedule (the
   fastest combination) and use VIA ACE if possible, but not under WinCE,
   both because it's possible that we could be running on an oddball
   embedded x86 that doesn't understand the necessary opcodes, and because
   eVC++ doesn't understand some of the extensions used in the ACE-support
   code - pcg */
...
/*  3. ASSEMBLER SUPPORT
    ...
    ASM_V1C  uses the assembler (aescrypt1.asm) with large tables for
             encryption and decryption and but with key scheduling in C
    ASM_V2   uses assembler (aescrypt2.asm) with compressed tables for
             encryption, decryption and key scheduling
    ASM_V2C	 uses assembler (aescrypt2.asm) with compressed tables for
             encryption and decryption and but with key scheduling in C
...
#if defined ( ASM_V1C ) || defined( ASM_V2 ) || defined( ASM_V2C )
#  if defined( _M_IX86 )
#    if 0 && !defined( ASM_V1C )
#      define ASM_V1C
...
#    endif
#  else
#    error Assembler code is only available for x86 systems
#  endif
#endif
...
#if (defined( ASM_V1C ) || defined( ASM_V2C )) && (ALGORITHM_BYTE_ORDER != PLATFORM_BYTE_ORDER)
...
#elif defined( ASSUME_VIA_ACE_PRESENT ) || defined( ASM_V1C )
...
#if ( FUNCS_IN_C & ENCRYPTION_IN_C ) || defined( ASM_V1C )
...
#if ( FUNCS_IN_C & DECRYPTION_IN_C ) || defined( ASM_V1C )
...
#if defined( ASM_V1C ) && defined( AES_DECRYPT ) && !defined( ISB_SET )
#define ISB_SET
#endif

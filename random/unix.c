...
*	Copyright Peter Gutmann, Paul Kendall, and Chris Wedgwood 1996-2013		*
*																			*
****************************************************************************/
...
#endif /* Linux on x86 */
#ifdef __OpenBSD__
  #include <sys/param.h>	/* For 'OpenBSD' version macro */
#endif /* __OpenBSD__ */
#include <signal.h>
...
#if defined( __OpenBSD__ ) && OpenBSD > 201412

CHECK_RETVAL \
static int getOpenBSDData( void )
	{
	RANDOM_STATE randomState;
	BYTE buffer[ 64 + 8 ];
	int quality = 0, status;

	/* OpenBSD also has the getentropy() function, but this was only added 
	   in 5.6 (conveniently about the time that LibreSSL needed it) which is 
	   far too new to safely include at this time */
	status = initRandomData( randomState, buffer, 64 );
	ENSURES_EXT( cryptStatusOK( status ), 0 );
	if( getentropy( buffer, 64 ) == 0 )
		{
		addRandomData( randomState, buffer, RANDOM_BUFSIZE );
		quality = 80;
		}

	...
	static const PACKET_NAME_INFO packetNameInfo[] = {
		...
		{ SSL_MSG_HANDSHAKE, "handshake (encrypted)" },
			/* We denote this one as an encrypted handshake packet rather 
			  ...
			  the inner contents, so if we ever get an handshake packet 
...
int processVersionInfo( INOUT SESSION_INFO *sessionInfoPtr, 
	...
	DEBUG_PRINT(( "Offered protocol version: 3.%d.\n", version ));
	switch( version )
		{
		case SSL_MINOR_VERSION_SSL:
			...
			retExt( CRYPT_ERROR_NOSECURE,
					( CRYPT_ERROR_NOSECURE, SESSION_ERRINFO, 
					  "Peer requested use of insecure SSL version 3, we can "
					  "only do TLS" ) );
#endif /* USE_SSL3 */
...
static int checkPacketHeader( INOUT SESSION_INFO *sessionInfoPtr, 
	...
	*packetLength = 0;
	...
	status = processVersionInfo( sessionInfoPtr, stream, NULL,
				( packetType == SSL_MSG_FIRST_HANDSHAKE ) ? TRUE : FALSE );
	if( cryptStatusError( status ) )
		return( status );
...
/* Process an alert packet.  IIS often just drops the connection rather than 
   ...
   times, and then restart the connection via an SSL resume.  Netscape-
   derived browsers in contrast just hope that the session won't time out 
   while waiting for the user to click OK.  As a result cryptlib sees a 
   closed connection and aborts the session setup process when talking to 
   MSIE, requiring a second call to the session setup to continue with the 
   resumed session */

...
static int ecdsaGetSPKI( const CRYPT_PKCINFO_ECC *ecdsaKey,
						 ...
						 OUT_LENGTH_SHORT_Z int *spkiLen )
	{
...
static int ecdsaInitKey( INOUT CONTEXT_INFO *contextInfoPtr, 
	...
	const BYTE *oid;
	int spkiSize, encodedPointSize = DUMMY_INIT, ecPointSize, cryptStatus;
	...
	/* Get the information that we'll need to specify the public-key 
	   parameters */
	oid = getECCOid( ecdsaKey->curveType );
	ENSURES( oid != NULL );
	...
	if( ecdsaKey->isPublicKey )
		{
		setTemplate( publicKeyTemplate, CKA_EC_PARAMS, oid, 
					 sizeofOID( oid ) );
		...
		{
		setTemplate( privateKeyTemplate, CKA_EC_PARAMS, oid, 
					 sizeofOID( oid ) );
...
static int ecdsaGenerateKey( INOUT CONTEXT_INFO *contextInfoPtr, 
	...
	ENSURES( cryptStatusOK( cryptStatus ) );
	oid = getECCOid( curveType );
	ENSURES( oid != NULL );
	setTemplate( publicKeyTemplate, CKA_EC_PARAMS, oid, sizeofOID( oid ) );

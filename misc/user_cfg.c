...
static const FIXED_OPTION_INFO FAR_BSS fixedOptionInfo[] = {
	...
	MK_OPTION_S( CRYPT_OPTION_INFO_COPYRIGHT, "Copyright Peter Gutmann, Eric Young, OpenSSL, 1994-2007", CRYPT_UNUSED ),
...
const char *getOptionString( OPTION_INFO *optionList,
...
int readConfig( const CRYPT_USER iCryptUser, const char *fileName,
	...
	int configFilePathLen, iterationCount = 0, status;
	...
	if( cryptStatusError( status ) )
		return( CRYPT_OK );		/* Can't build config path */
...
int prepareConfigData( OPTION_INFO *optionList, const char *fileName,
	...
	*length = 0;
	...
	if( ( *data = clAlloc( "prepareConfigData", *length ) ) == NULL )
		return( CRYPT_ERROR_MEMORY );
